...
from structure_tools.Sim_ideogram_tools import (
	compress_ideo, class_and_ideo, return_ideogram
	)

def quick_kdeClass(genotype, summary,Names_idx,RG_info, Chr= 6,subset= [], ref_labels= [0,1,2], n_comps= 4, ID_col= "IRIS_ID",
                  fixed= True,window_size= 120, Steps= 10,
                  Outlier_threshold= 1e-5,Comparison_threshold= 4,Sn= 90, repn= 20,
                  id_tag= 'kde', color_lookup= {}, square= False, start= 0, end= 0):
    ...
    Windows_profiles, var_comp_store= KDE_window_profiles(genotype,Windows,Names_idx,
                        ...
                        repn= repn, code= code,others= 'admx',Sn= Sn, same= True)
    ...
    ID= 'kde{}_gp{}_w{}_N{}_M{}_th{}'.format(id_tag,'-'.join([str(x) for x in groups_plot]),Sn,Sm,window_size,Comparison_threshold)  
